...
import bubble.game.BubbleMovable;
...
@Getter
@Setter
public class Bubble extends JLabel implements BubbleMovable {
		...
		private Enemy hitEnemy(List<Enemy> enemyList) {
		...
		public void left() {
			...
			for(int i = 0; i<400; i++) {
				...
				Enemy hitEnemy = hitEnemy(enemyList);
				if(hitEnemy != null) {
					bubbleHit(hitEnemy);
					break;
				}
		...
		@Override
		public void bubbleHit(Enemy enemy) {
			state = 1;
			enemy.setState(1);
			setIcon(bubbled);
